...
# Script for the final report paper to show Grad-CAM on example pictures
...
def emotion_detection(frame):
    ...
    for (x, y, w, h) in faces:
        ...
        # Emotion label + confidence above the face box
...
result_frame, probs = emotion_detection(img_bgr)

# Add probability bars onto the output image
if probs is not None:
    draw_emotion_bars(result_frame, probs)

output = current_folder / "outputimage.png"
cv2.imwrite(str(output), result_frame)
